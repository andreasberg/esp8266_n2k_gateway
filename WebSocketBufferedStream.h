#ifndef WebSocketBufferedStream_h
#define WebSocketBufferedStream_h
...
#define WSOUT_BOTH 0           // write to both _websock and _stream
#define WSOUT_WEBSOCKONLY 1    // write only to _websock
#define WSOUT_STREAMONLY 2     // write only to  _stream

#define WSBUF_SIZE 20

class WebSocketBufferedStream : public Stream
{
  Stream            *_stream;
  WebSocketsServer  *_websock;
  uint8_t           _wsout;
  char              _wsbuf[WSBUF_SIZE];
  size_t            _wsbufidx;

private:
  void _wsflush() {
    _websock->broadcastTXT((const char *)&_wsbuf,_wsbufidx);
    _wsbufidx = 0;
  }
...
public:
  ...
  WebSocketBufferedStream( WebSocketsServer *websock, Stream *stream, uint8_t wsout)
    {
      _websock = websock;
      _stream = stream;
      _wsout = wsout;
  ...
  WebSocketBufferedStream( WebSocketsServer *websock, Stream *stream) 
    {
      _websock = websock;
      _stream = stream;
      _wsout = WSOUT_BOTH;
  ...
  WebSocketBufferedStream( WebSocketsServer *websock)
    {
      _websock = websock;
      _stream = NULL;
      _wsout = WSOUT_WEBSOCKONLY;
      _wsbufidx = 0;
    }
  ...
  virtual size_t write(uint8_t c)
   {
     ...
     size_t written = 0;
     if (_wsout == WSOUT_BOTH || _wsout == WSOUT_STREAMONLY) 
         written = _stream->write( c );
      //then write to websocket
     if (_wsout == WSOUT_BOTH || _wsout == WSOUT_WEBSOCKONLY) {
        //_websock->broadcastTXT(&c,1);
        _wsbuf[_wsbufidx++] = c;
        if (_wsbufidx == WSBUF_SIZE) 
          _wsflush();
        written++;
     }
...
#endif  // WebSocketBufferedStream_h
